...
export const SponsorCardList: React.FC<SponsorCardListProps> = ({
  ...
  const { ratedSponsorCards, originalCount } = useMemo(() => {
    const mappedSponsors: ISponsorCard[] = filteredSponsors.map((sponsor) => ({
      id: sponsor.id,
      sponsorCard: sponsor,
    }));
    ...
    switch (sortOrder) {
      case SortOrder.ID_ASC:
        mappedSponsors.sort((a, b) => a.id.localeCompare(b.id));
        break;
      case SortOrder.ID_DESC:
        mappedSponsors.sort((a, b) => b.id.localeCompare(a.id));
        break;
    }

    const slicedSponsors =
      maxNum !== undefined
        ? mappedSponsors.slice(0, maxNum)
        : mappedSponsors;

    return {
      ratedSponsorCards: slicedSponsors,
      originalCount: mappedSponsors.length,
    };
  }, [filteredSponsors, sortOrder, maxNum]);
